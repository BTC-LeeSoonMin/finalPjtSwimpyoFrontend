...
const UserDetailRoom = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            console.log("sendToResData!!!", sendToResData);
            try {
                const res = await api.post(`http://43.203.71.198/api/user/room/showRoomDetail?a_r_no=${roomNum.a_r_no}`);
                //  res -> 서버에서 받아온 데이터
    ...
    const fetchDataFromBack = async () => {
        ...
        try {
            ...
            const res = await api.post(`http://43.203.71.198/api/user/reservation/ready`, data, {
                headers: {
